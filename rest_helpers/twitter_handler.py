...
TWITTER_FIELD_TOGGLES = quote("""{"withArticleRichContentState":false}""")

TWITTER_FEATURES = quote("""{"rweb_lists_timeline_redesign_enabled":true,"responsive_web_graphql_exclude_directive_enabled":true,"verified_phone_label_enabled":false,"creator_subscriptions_tweet_preview_api_enabled":true,"responsive_web_graphql_timeline_navigation_enabled":true,"responsive_web_graphql_skip_user_profile_image_extensions_enabled":false,"tweetypie_unmention_optimization_enabled":true,"responsive_web_edit_tweet_api_enabled":true,"graphql_is_translatable_rweb_tweet_is_translatable_enabled":true,"view_counts_everywhere_api_enabled":true,"longform_notetweets_consumption_enabled":true,"responsive_web_twitter_article_tweet_consumption_enabled":false,"tweet_awards_web_tipping_enabled":false,"freedom_of_speech_not_reach_fetch_enabled":true,"standardized_nudges_misinfo":true,"tweet_with_visibility_results_prefer_gql_limited_actions_policy_enabled":true,"longform_notetweets_rich_text_read_enabled":true,"longform_notetweets_inline_media_enabled":true,"responsive_web_media_download_video_enabled":false,"responsive_web_enhance_cards_enabled":false}""")
...
def get_twitter_searches(search_term: str, return_count: int) -> List[object]:
    """get search results for twitter"""
    request_variables: TwitterRequestVariables = {"rawQuery": search_term, "count": return_count,
                                                  "querySource": "typed_query", "product": "Latest", "cursor": ""}
    json_string: str = json.dumps(request_variables)
    variable_encoded: str = quote(string=json_string, encoding="utf-8")
    url = f"https://twitter.com/i/api/graphql/L1VfBERtzc3VkBBT0YAYHA/SearchTimeline?variables={variable_encoded}&features={TWITTER_FEATURES}&fieldToggles={TWITTER_FIELD_TOGGLES}"
    response = get(url=url, headers=TWITTER_SECRETS, timeout=10000)
    return response.json()["data"]["search_by_raw_query"]["search_timeline"]["timeline"]["instructions"][0]["entries"]
...
if __name__ == "__main__":
    ...
    print(get_twitter_searches("bye", 50))
